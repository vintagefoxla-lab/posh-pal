...
const Dashboard = ({ tools, setActiveTab, isPro }) => (
  <div className="space-y-6 fade-in-up">
    ...
    {/* Social Proof (Marketing Asset) */}
    {!isPro && (
      <SocialProofGrid className="mt-4 mb-8" />
    )}
...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (params.get('success') === 'true') {
      userFetch('/api/stripe/success', { method: 'POST' })
        .then(res => res.json())
        .then(data => {
          if (data.success) {
            setIsPro(true)
        ...
        .catch(err => console.error('Failed to confirm subscription:', err))
        .finally(() => {
  ...
  const tools = [
    ...
    { id: 'media-kit', name: 'Media Kit', icon: Sparkles, color: 'text-brand-600', bg: 'bg-brand-50', desc: 'Social media assets' },
    { id: 'cms', name: 'Blog Manager', icon: FileText, color: 'text-rose-600', bg: 'bg-rose-50', desc: 'Admin Content CMS', adminOnly: true },
  ]
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      default: return <Dashboard tools={tools.filter(t => !t.adminOnly || userId === 'agent-lead')} setActiveTab={setActiveTab} isPro={isPro} userFetch={userFetch} />
    }
  }
