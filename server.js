...
const runQuery = (query, retries = 3) => {
  for (let i = 0; i < retries; i++) {
    try {
      const output = execSync(`team-db "${query.replace(/"/g, '\\"')}"`).toString();
      return JSON.parse(output);
    } catch (error) {
      if (error.stderr && error.stderr.toString().includes('Locked') && i < retries - 1) {
        console.warn(`Database locked, retrying (${i + 1}/${retries})...`);
        execSync('sleep 0.5');
        continue;
      }
      console.error('Database Error:', error.stderr ? error.stderr.toString() : error.message);
      throw error;
    }
  }
};
...
app.get('/api/analytics/summary', (req, res) => {
  ...
  try {
    if (days && !isNaN(days)) {
      const historical = runQuery(`SELECT * FROM daily_stats WHERE user_id = '${escapeSql(req.userId)}' ORDER BY date DESC LIMIT ${parseInt(days)}`);
      return res.json(historical);
    }

    const stats = runQuery(`SELECT shares_count FROM bot_stats WHERE user_id = '${escapeSql(req.userId)}'`);
    const offers = runQuery(`SELECT status, bundle_price, discount_percent, total_value, item_count, type, inventory_id FROM offers WHERE user_id = '${escapeSql(req.userId)}'`);
    
    let totalSent = 0;
    let totalAccepted = 0;
    let totalRevenue = 0;
    
    const inventory = runQuery(`SELECT id, price FROM inventory WHERE user_id = '${escapeSql(req.userId)}'`);
    const invMap = {};
...
app.get('/api/analytics/attribution', (req, res) => {
  try {
    ...
    // Group by source for simplified view
    const summary = {};
    stats.forEach(row => {
    ...
    // Calculate revenue per source by joining with users and their activity
    const revenueBySource = runQuery(`
      SELECT u.utm_source, SUM(ds.revenue) as total_revenue
      FROM users u
...
app.get('/api/analytics/leaderboard', (req, res) => {
  try {
    const topReferrers = runQuery(`
      SELECT name, handle, referral_code as code, referral_count as referrals
      FROM users 
...
setInterval(() => {
  try {
    ...
    if (Math.random() < source.conversionRate) {
      ...
      // Create user
      runQuery(`INSERT INTO users (id, referral_code, utm_source, utm_medium, utm_campaign) 
                VALUES ('${simulatedUserId}', '${simulatedUserId.toUpperCase()}123', ${val(source.source)}, ${val(source.medium)}, ${val(source.campaign)})`);
